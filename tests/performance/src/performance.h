...
class Timer
{
  public:
    ...
    inline auto now() const noexcept { return std::chrono::steady_clock::now(); }
  ...
  protected:
    inline void reset_timer() noexcept { start = end = {}; }

    inline void start_timer() noexcept { start = now(); }

    inline void finish_timer() noexcept { end = now(); }
};
...
class Output
{
  ...
  protected:
    inline void print_line_separator() noexcept { std::cout << separator << std::endl; }
