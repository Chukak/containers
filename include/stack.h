    ...
    _empty(orig._empty)
{
    if (!_empty) {
        ...
        while (temp) {
            t->prev = make_node_ptr(Node(temp->value, NULL));
            temp = temp->prev;
            t = t->prev;
        }
    }
...
{
    node_ptr new_node = make_shared_ptr<Node>(Node(std::forward(element), NULL)); // a new pointer.
    if (!_empty) {
 ...
 */
template<typename Type>
Stack<Type>::iterator::iterator(node_ptr node) :
    _node(node)
{}
